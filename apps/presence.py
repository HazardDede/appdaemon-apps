...
class Validator:
  ...
  SCHEMA = Schema({
    ...
    Optional('just_left_delay', default=T5_MINUTES): int,
    Optional('just_arrived_delay', default=T5_MINUTES): int,
    Optional('extended_away_delay', default=T24_HOURS): int
  }, extra=True)
...
class App(hass.Hass):
  ...
  def _init_current_state(self):
    ...
    if tracker_state is TrackerState.Home and curstate not in (State.JustArrived, State.Home):
      ...
      self.log("Current state '{curstate}' is invalid with tracker state '{tracker_state}'. Resetting to away".format(**locals()))
      curstate = State.Away
